...
void pad_cpu(RealNum* data, const int channels,
  ...
  memset(data_padded, 0, pad_height * pad_width * channels * sizeof(RealNum));

  for (int channel = 0; channel < channels; channel++) {
    RealNum* px = data_padded + (pad_width * pad_top + pad_left) + channel * (pad_width * pad_height);
    for (int y = 0; y < height; y++) {
      memcpy(px, data, sizeof(RealNum) * width);
      px += pad_width;
      data += width;
    }
  }
